...
int main(int argc, char* argv[]) {
	...
	*/
	...
	/*
		...
		(wnd code means find window with the provided argument somewhere in the title)
		(wnx code means that the window title must exactly match the provided arg)
	*/
		...
		for (std::string wndName : windowList) {
			if (exclusiveSearch) {
				if (wndName.compare(searchName) == 0) {
					HWND thisWnd = FindWindowA(NULL, wndName.c_str());
					util::FrontWindow(thisWnd);
				}
			}
			else {
				if (wndName.find(searchName) != std::string::npos) {
					HWND thisWnd = FindWindowA(NULL, wndName.c_str());
					util::FrontWindow(thisWnd);
				}
			}
